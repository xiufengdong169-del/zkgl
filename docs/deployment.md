...
函数包生成在 `functions/zkgl-api`、`functions/zkgl-reminder` 与 `functions/zkgl-export-worker`，均为构建产物且不纳入版本管理。上述命令部署的是事件函数，不要添加 `--httpFn`；前端使用时需在 CloudBase 控制台仅为 `zkgl-api` 配置 HTTP 访问服务路径，并将完整访问地址写入 `VITE_API_BASE_URL`。
...
`zkgl-export-worker` 使用 CloudBase 定时触发器 `zkglExportWorker`，建议每 5 分钟触发一次，例如七段 Cron `0 */5 * * * * *`。只允许定时触发器调用 `zkgl-export-worker`，不要为它配置客户端 HTTP 访问路径。
