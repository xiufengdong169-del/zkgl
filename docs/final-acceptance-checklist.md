...
- [ ] API 测试通过，当前基线为 83 个测试文件 / 412 条测试。
...
- [ ] `npm run verify:local-demo` 通过：demo 模式前端包可在临时 HTTP 服务下打开，主要 SPA 路由均返回众肯系统前端壳，且前端入口 JS 模块和 CSS 样式资源可访问。
...
- [ ] 如服务器登录权限暂不可用，可先执行 `npm run verify:local-demo`，确认 demo 模式前端包在本地临时 HTTP 服务下可打开，且 `/`、`/projects`、`/contracts`、`/finance` 和 `/admin` 路由均返回众肯系统前端壳，前端入口 JS 模块和 CSS 样式资源可访问。
...
- [ ] 无正式合同不能正常启动项目；提前启动必须审批并持续生成补签提醒，超过预计签约日后持续生成异常提醒。
