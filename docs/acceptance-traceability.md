...
- `npm run test`：API 87 个测试文件 / 464 条测试通过；Web 10 个测试文件 / 52 条测试通过。
- `npm run build`：通过。
...
- `npm run verify:object-restore`：项目附件与后台导出文件恢复清单示例通过，覆盖 `private/files`、`private/exports`、HTTPS 下载抽查、敏感附件拒绝和过期导出拒绝证据。
