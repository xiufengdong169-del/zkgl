...
| AC-12 | 无关项目详情、导出和附件地址均按数据范围拒绝并审计；附件上传完成必须严格匹配后端预分配私有路径 | `apps/api/src/handler.test.ts`、`apps/api/src/files.test.ts`、`apps/api/src/file-upload-complete.test.ts` |
...
- `npm run test`：API 88 个测试文件 / 471 条测试通过；Web 10 个测试文件 / 57 条测试通过。
- `npm run build`：通过。
...
- `node scripts/verify-backup-assets.mjs`：MySQL 8.0 备份脚本、备份 timer、对象恢复清单校验器和备份恢复验收文档通过。
- `npm run verify:object-restore`：项目附件与后台导出文件恢复清单示例通过，覆盖 `private/files`、`private/exports`、同一业务时间窗口、HTTPS 下载抽查、敏感附件拒绝和过期导出拒绝证据。
- `npm run verify:initialization-data`：由 `scripts/verify-initialization-data.mjs` 校验 `docs/initialization-data.example.json`，上线初始化资料清单示例通过，覆盖部门、人员、CloudBase UID、角色分配、审批岗位、审批金额阈值、编号规则、系统参数和演示账号；示例中的角色、审批模板、编号规则和系统参数代码与 `database/init/schema.sql` 空库种子一致。
- `node scripts/verify-server-preflight.mjs`：Tencent Cloud Lighthouse 上线预检资产通过。
- `npm run verify:local-demo`：本地 demo 模式构建、临时 HTTP 服务、全部声明的前端 SPA 路由壳、前端入口 JS 模块、CSS 样式资源和全部构建静态资源校验通过，可在服务器权限未就绪时先证明可视化演示包可打开。
...
- 本地阶段完成报告见 `docs/local-development-completion-report.md`，用于区分本地自动化已完成事项与仍需在腾讯云轻量服务器真实环境完成的部署、现场压测和备份恢复演练事项。
