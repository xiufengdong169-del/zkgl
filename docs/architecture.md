...
当前阶段不维护数据库迁移。开发、测试和首次上线环境均从空库执行 `database/init/schema.sql`。该初始化脚本不得包含 `DROP`、`TRUNCATE`、`ALTER TABLE`、`CREATE DATABASE` 或 `USE` 等破坏性、迁移式或绑定具体库名的语句。正式投产后如需结构变更，应先评审变更影响，再建立生产变更机制。
