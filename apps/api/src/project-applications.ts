...
export const projectApplicationInput = z.object({
...
export type ProjectApplicationAction = 'SUBMIT' | 'RETURN' | 'REJECT' | 'WITHDRAW'

const transitions: Record<ProjectApplicationStatus, Partial<Record<ProjectApplicationAction, ProjectApplicationStatus>>> = {
  DRAFT: { SUBMIT: 'APPROVAL_PENDING' },
  APPROVAL_PENDING: { RETURN: 'RETURNED', REJECT: 'REJECTED', WITHDRAW: 'WITHDRAWN' },
...
export function transitionProjectApplication(
  ...
  if (!next) {
    throw new AppError('INVALID_APPLICATION_TRANSITION', `立项申请状态 ${status} 不允许执行 ${action}`, 409)
  }
  return next
}
...
export interface ApprovalRepository {
  ...
  createProjectFromApplication(application: ProjectApplicationRecord, projectCode: string, operatorId: string): Promise<ProjectRecord>
  markApplicationApproved(id: string, operatorId: string): Promise<void>
  copySuggestedMembers(applicationId: string, projectId: string, operatorId: string): Promise<void>
}
...
export async function approveProjectApplication(
  ...
  return repository.transaction(async () => {
    ...
    if (!application) throw new AppError('APPLICATION_NOT_FOUND', '立项申请不存在', 404)

    const existingProject = await repository.findProjectByApplication(applicationId)
    if (existingProject) return existingProject

    if (application.status !== 'APPROVAL_PENDING') {
      throw new AppError('APPLICATION_NOT_PENDING', '仅审批中的立项申请可通过', 409)
    }

    const projectCode = await repository.allocateNumber('PROJECT')
    const project = await repository.createProjectFromApplication(application, projectCode, operatorId)
    await repository.copySuggestedMembers(applicationId, project.id, operatorId)
