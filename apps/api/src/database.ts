...
interface SensitiveRow extends RowDataPacket {
  fieldCode: string;
  accessLevel: string;
  explicitDeny: number;
}
...
function mapScope(
  ...
  switch (row.scope_type) {
    ...
    case "OWNER":
      return { type: "OWNER", userId };
    case "CREATOR":
      return { type: "CREATOR", userId };
    case "PARTICIPANT":
      return { type: "PARTICIPANT", userId };
    case "DEPARTMENT":
      return {
        type: "DEPARTMENT",
        departmentIds: [row.scope_value || departmentId],
      };
    case "PROJECT":
      return row.scope_value
        ? { type: "PROJECT", projectIds: [row.scope_value] }
        : null;
    default:
      return null;
  }
...
export async function findSessionUserByCloudbaseUid(
  pool: Pool,
  uid: string,
  ...
  const [permissions] = await pool.execute<CodeRow[]>(
    `SELECT DISTINCT p.code FROM iam_permission p JOIN iam_role_permission rp ON rp.permission_id=p.id
      JOIN iam_user_role ur ON ur.role_id=rp.role_id WHERE ur.user_id=?`,
    [user.id],
  );
  const [scopes] = await pool.execute<ScopeRow[]>(
    `SELECT s.scope_type,s.scope_value FROM iam_role_data_scope s JOIN iam_user_role ur ON ur.role_id=s.role_id
      WHERE ur.user_id=? AND s.status='ENABLED'`,
    [user.id],
  );
  const [sensitiveGrants] = await pool.execute<SensitiveRow[]>(
    `SELECT g.field_code fieldCode,g.access_level accessLevel,g.explicit_deny explicitDeny FROM iam_sensitive_field_grant g JOIN iam_user_role ur ON ur.role_id=g.role_id WHERE ur.user_id=? AND g.status='ENABLED'`,
    [user.id],
  );
  return {
    id: user.id,
    cloudbaseUid: user.cloudbase_uid,
    ...
    dataScopes: scopes
      .map((row) => mapScope(row, user.id, user.department_id))
      .filter((scope): scope is DataScope => scope !== null),
  };
}
...
export function assertAffectedRows(count: number, message: string): void {
  if (count !== 1)
    throw new AppError("CONCURRENT_WRITE_CONFLICT", message, 409);
}
