...
function makeValidInputs(expected: DeploymentConfigModule["expected"]): DeploymentInputs {
  ...
  return {
    ...
    packageJson: {
      ...
      scripts: {
        verify:
          "npm run typecheck && npm run verify:deployment-config && npm run verify:performance-acceptance && npm run verify:object-restore && npm run verify:initialization-data && npm run verify:local-demo",
...
describe("deployment config verifier script", () => {
  ...
  it("rejects removing local visual demo verification from the primary command", async () => {
    ...
    const inputs = makeValidInputs(expected);
    inputs.packageJson.scripts!.verify =
      "npm run typecheck && npm run verify:deployment-config && npm run verify:performance-acceptance && npm run verify:object-restore && npm run verify:initialization-data";
