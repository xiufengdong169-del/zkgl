...
export class MySqlActionExecutor {
  ...
  async execute(action: string, value: unknown, user: SessionUser): Promise<unknown> {
    ...
    return withTransaction(this.pool, async (connection) => {
      switch (action) {
        case 'message.list': {
          const page=input.page as number,pageSize=input.pageSize as number,[rows]=await connection.execute<RowDataPacket[]>(`SELECT id,message_type messageType,title,content,business_type businessType,business_id businessId,read_at readAt,created_at createdAt FROM sys_message WHERE recipient_id=? ORDER BY created_at DESC LIMIT ? OFFSET ?`,[user.employeeId,pageSize,(page-1)*pageSize]);return{items:rows,page,pageSize}
        }
        case 'reminder.refresh': {
          const statements=[`INSERT INTO sys_message(recipient_id,message_type,title,content,business_type,business_id) SELECT c.owner_id,'CONTRACT_EXPIRY',CONCAT('合同即将到期：',c.contract_name),CONCAT('到期日：',c.expires_on),'CONTRACT',c.id FROM con_contract c WHERE c.is_deleted=0 AND c.status IN('PENDING_SIGNATURE','PERFORMING') AND c.expires_on BETWEEN CURDATE() AND DATE_ADD(CURDATE(),INTERVAL 30 DAY) AND NOT EXISTS(SELECT 1 FROM sys_message m WHERE m.recipient_id=c.owner_id AND m.message_type='CONTRACT_EXPIRY' AND m.business_id=c.id AND m.created_at>=DATE_SUB(NOW(),INTERVAL 7 DAY))`,`INSERT INTO sys_message(recipient_id,message_type,title,content,business_type,business_id) SELECT b.business_owner_id,'BID_DEADLINE',CONCAT('投标即将截止：',b.bid_code),CONCAT('截止时间：',b.deadline_at),'BID_APPLICATION',b.id FROM bid_application b WHERE b.is_deleted=0 AND b.status NOT IN('WON','LOST','FAILED','ABANDONED') AND b.deadline_at BETWEEN NOW() AND DATE_ADD(NOW(),INTERVAL 7 DAY) AND NOT EXISTS(SELECT 1 FROM sys_message m WHERE m.recipient_id=b.business_owner_id AND m.message_type='BID_DEADLINE' AND m.business_id=b.id AND m.created_at>=DATE_SUB(NOW(),INTERVAL 2 DAY))`,`INSERT INTO sys_message(recipient_id,message_type,title,content,business_type,business_id) SELECT p.project_manager_id,'DEPOSIT_OVERDUE',CONCAT('保证金退回逾期：',g.deposit_code),CONCAT('应退日：',g.due_return_on),'DEPOSIT',g.id FROM fin_deposit g JOIN prj_project p ON p.id=g.project_id WHERE g.due_return_on<CURDATE() AND g.occupied_amount>0 AND g.status NOT IN('RETURNED','VOID') AND NOT EXISTS(SELECT 1 FROM sys_message m WHERE m.recipient_id=p.project_manager_id AND m.message_type='DEPOSIT_OVERDUE' AND m.business_id=g.id AND m.created_at>=DATE_SUB(NOW(),INTERVAL 7 DAY))`,`INSERT INTO sys_message(recipient_id,message_type,title,content,business_type,business_id) SELECT r.owner_id,'RISK_DUE',CONCAT('问题风险待处理：',r.title),CONCAT('计划解决日：',r.planned_resolution_on),'RISK',r.id FROM prj_risk_issue r WHERE r.is_deleted=0 AND r.status<>'CLOSED' AND r.planned_resolution_on<=DATE_ADD(CURDATE(),INTERVAL 3 DAY) AND NOT EXISTS(SELECT 1 FROM sys_message m WHERE m.recipient_id=r.owner_id AND m.message_type='RISK_DUE' AND m.business_id=r.id AND m.created_at>=DATE_SUB(NOW(),INTERVAL 3 DAY))`];let created=0;for(const sql of statements){const[result]=await connection.execute<ResultSetHeader>(sql);created+=result.affectedRows}return{created}
        }
        case 'report.dashboard': {
          const all=user.dataScopes.some((scope)=>scope.type==='ALL'),access=`(?=1 OR p.project_manager_id=? OR EXISTS(SELECT 1 FROM prj_project_member m WHERE m.project_id=p.id AND m.employee_id=? AND m.status='ACTIVE'))`;const[rows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(p.estimated_revenue-p.estimated_cost),0) expectedProfit,COALESCE(SUM((SELECT COALESCE(SUM(c.tax_exclusive_amount),0) FROM con_contract c WHERE c.project_id=p.id AND c.contract_type='INCOME' AND c.amount_status='CONFIRMED' AND c.status IN('PENDING_SIGNATURE','PERFORMING','COMPLETED') AND c.is_deleted=0)-(SELECT COALESCE(SUM(d.amount),0) FROM fin_reimbursement_detail d JOIN fin_reimbursement h ON h.id=d.reimbursement_id WHERE h.project_id=p.id AND h.approval_status='APPROVED' AND d.status='ACTIVE')-(SELECT COALESCE(SUM(s.confirmed_cost_amount),0) FROM partner_settlement s WHERE s.project_id=p.id AND s.status IN('APPROVED','PAID'))-(SELECT COALESCE(SUM(g.loss_confirmed_amount),0) FROM fin_deposit g WHERE g.project_id=p.id)),0) contractOperatingProfit,COALESCE(SUM((SELECT COALESCE(SUM(r.amount),0) FROM fin_receipt r WHERE r.project_id=p.id AND r.status='ACTIVE')-(SELECT COALESCE(SUM(x.amount),0) FROM fin_payment_detail x WHERE x.project_id=p.id AND x.status='ACTIVE')),0) cashContribution,COUNT(*) projectCount FROM prj_project p WHERE p.is_deleted=0 AND ${access}`,[all?1:0,user.employeeId,user.employeeId]);return{...rows[0],disclaimer:'内部项目经营口径，不属于会计利润'}
        }
        case 'report.project.export': {
          const all=user.dataScopes.some((scope)=>scope.type==='ALL'),access=`(?=1 OR p.project_manager_id=? OR EXISTS(SELECT 1 FROM prj_project_member m WHERE m.project_id=p.id AND m.employee_id=? AND m.status='ACTIVE'))`;const[rows]=await connection.execute<RowDataPacket[]>(`SELECT p.project_code projectCode,p.project_name projectName,c.name customerName,p.status,p.estimated_revenue estimatedRevenue,p.estimated_cost estimatedCost,(SELECT COALESCE(SUM(x.tax_exclusive_amount),0) FROM con_contract x WHERE x.project_id=p.id AND x.contract_type='INCOME' AND x.amount_status='CONFIRMED' AND x.is_deleted=0) confirmedIncome,(SELECT COALESCE(SUM(r.amount),0) FROM fin_receipt r WHERE r.project_id=p.id AND r.status='ACTIVE') receivedAmount FROM prj_project p JOIN crm_counterparty c ON c.id=p.customer_id WHERE p.is_deleted=0 AND ${access} ORDER BY p.id DESC LIMIT 1001`,[all?1:0,user.employeeId,user.employeeId]);if(rows.length>1000)throw new AppError('EXPORT_BACKGROUND_REQUIRED','超过 1000 条的导出必须转后台任务',409);return{rows,disclaimer:'内部项目经营口径，不属于会计利润'}
        }
        case 'admin.overview': {
          const[departments]=await connection.execute<RowDataPacket[]>(`SELECT id,code,name,status FROM org_department WHERE is_deleted=0 ORDER BY id`),[employees]=await connection.execute<RowDataPacket[]>(`SELECT e.id,e.employee_code employeeCode,e.name,e.employee_type employeeType,e.department_id departmentId,d.name departmentName,e.position_name positionName,e.account_status accountStatus FROM org_employee e JOIN org_department d ON d.id=e.department_id WHERE e.is_deleted=0 ORDER BY e.id DESC LIMIT 200`),[roles]=await connection.execute<RowDataPacket[]>(`SELECT id,code,name,status FROM iam_role WHERE is_deleted=0 ORDER BY id`),[users]=await connection.execute<RowDataPacket[]>(`SELECT u.id,u.username,u.cloudbase_uid cloudbaseUid,u.status,e.name employeeName,GROUP_CONCAT(r.name ORDER BY r.id SEPARATOR '、') roleNames,GROUP_CONCAT(r.id ORDER BY r.id) roleIds FROM iam_user u JOIN org_employee e ON e.id=u.employee_id LEFT JOIN iam_user_role ur ON ur.user_id=u.id LEFT JOIN iam_role r ON r.id=ur.role_id WHERE u.is_deleted=0 GROUP BY u.id ORDER BY u.id DESC LIMIT 200`);return{departments,employees,roles,users}
        }
        case 'admin.department.create': {
          const[result]=await connection.execute<ResultSetHeader>(`INSERT INTO org_department(code,name) VALUES(?,?)`,[input.code,input.name]);return{id:String(result.insertId)}
        }
        case 'admin.employee.create': {
          const[result]=await connection.execute<ResultSetHeader>(`INSERT INTO org_employee(employee_code,name,employee_type,department_id,position_name,mobile,email,joined_on,created_by,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?)`,[input.employeeCode,input.name,input.employeeType,input.departmentId,input.positionName??null,input.mobile??null,input.email??null,input.joinedOn??null,user.id,user.id]);return{id:String(result.insertId)}
        }
        case 'admin.user.role.set': {
          const[current]=await connection.execute<RowDataPacket[]>(`SELECT 1 FROM iam_user_role ur JOIN iam_role r ON r.id=ur.role_id WHERE ur.user_id=? AND r.code='ADMIN'`,[input.userId]),[adminRole]=await connection.execute<RowDataPacket[]>(`SELECT id FROM iam_role WHERE code='ADMIN'`);const requestedAdmin=(input.roleIds as string[]).includes(String(adminRole[0]?.id??''));if(current[0]&&!requestedAdmin){const[count]=await connection.execute<RowDataPacket[]>(`SELECT COUNT(DISTINCT ur.user_id) count FROM iam_user_role ur JOIN iam_role r ON r.id=ur.role_id JOIN iam_user u ON u.id=ur.user_id WHERE r.code='ADMIN' AND u.status='ENABLED' AND u.is_deleted=0`);if(Number(count[0]?.count??0)<=1)throw new AppError('LAST_ADMIN_REQUIRED','系统至少必须保留一名启用的管理员',409)}await connection.execute(`DELETE FROM iam_user_role WHERE user_id=?`,[input.userId]);for(const roleId of input.roleIds as string[])await connection.execute(`INSERT INTO iam_user_role(user_id,role_id) VALUES(?,?)`,[input.userId,roleId]);return{userId:input.userId,roleCount:input.roleIds.length}
        }
        case 'file.list': {
        ...
        case 'partner.plan.create': {
          if(input.settlementMethod==='RATIO'){
            const [ratioRows]=await connection.execute<RowDataPacket[]>(`SELECT v.ratio FROM partner_plan p JOIN partner_plan_version v ON v.plan_id=p.id AND v.status='ENABLED' WHERE p.project_id=? AND p.status='ENABLED' AND v.calculation_basis=? FOR UPDATE`,[input.projectId,input.calculationBasis])
            const ratioTotal=ratioRows.reduce((sum,row)=>sum+Number(row.ratio??0),0)
            if(ratioTotal+Number(input.ratio)>1.0000001)throw new AppError('PARTNER_RATIO_EXCEEDED','同一项目同一基数的合作比例合计不得超过 100%',409)
          }
          const code=await allocateNumber(connection,'PARTNER_PLAN');const[plan]=await connection.execute<ResultSetHeader>(`INSERT INTO partner_plan(project_id,partner_id,plan_code,owner_id,created_by,updated_by) VALUES(?,?,?,?,?,?)`,[input.projectId,input.partnerId,code,input.ownerId,user.id,user.id])
          await connection.execute(`INSERT INTO partner_plan_version(plan_id,version_number,settlement_method,fixed_amount,ratio,calculation_basis,deductible_cost_scope,upper_limit,lower_limit,effective_from,effective_to,conditions,rounding_rule) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,[plan.insertId,1,input.settlementMethod,input.fixedAmount??null,input.ratio??null,input.calculationBasis,JSON.stringify(input.deductibleCostScope),input.upperLimit??null,input.lowerLimit??null,input.effectiveFrom,input.effectiveTo??null,input.conditions??null,'ROUND_HALF_UP'])
          return{id:String(plan.insertId),code,version:1}
        }
        case 'partner.settlement.create': {
          const [plans]=await connection.execute<RowDataPacket[]>(`SELECT p.id planId,p.project_id projectId,p.partner_id partnerId,v.id versionId,v.version_number versionNumber,v.settlement_method settlementMethod,v.fixed_amount fixedAmount,v.ratio,v.calculation_basis basis,v.deductible_cost_scope deductibleScope,v.upper_limit upperLimit,v.lower_limit lowerLimit,v.rounding_rule roundingRule FROM partner_plan p JOIN partner_plan_version v ON v.plan_id=p.id WHERE p.id=? AND p.status IN('ENABLED','DRAFT') AND v.status IN('ENABLED','DRAFT') AND v.effective_from<=? AND (v.effective_to IS NULL OR v.effective_to>=?) ORDER BY v.version_number DESC LIMIT 1 FOR UPDATE`,[input.planId,input.periodEndOn,input.periodStartOn]);const plan=plans[0]
          if(!plan)throw new AppError('PARTNER_PLAN_NOT_EFFECTIVE','结算期间没有有效合作方案',409)
          let basisAmount=0
          if(plan.basis==='CONTRACT_REVENUE_EX_TAX'){const[r]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(tax_exclusive_amount),0) amount FROM con_contract WHERE project_id=? AND contract_type='INCOME' AND amount_status='CONFIRMED' AND status NOT IN('VOID','REJECTED')`,[plan.projectId]);basisAmount=Number(r[0]?.amount??0)}
          else if(plan.basis==='ACTUAL_RECEIPTS'){const[r]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(amount),0) amount FROM fin_receipt WHERE project_id=? AND status='ACTIVE' AND received_on<=?`,[plan.projectId,input.periodEndOn]);basisAmount=Number(r[0]?.amount??0)}
          else if(plan.basis==='PROJECT_GROSS_PROFIT'){
            const[incomeRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(tax_exclusive_amount),0) amount FROM con_contract WHERE project_id=? AND contract_type='INCOME' AND amount_status='CONFIRMED' AND status NOT IN('VOID','REJECTED')`,[plan.projectId])
            const[costRows]=await connection.execute<RowDataPacket[]>(`SELECT (COALESCE((SELECT SUM(d.amount) FROM fin_reimbursement_detail d JOIN fin_reimbursement h ON h.id=d.reimbursement_id WHERE h.project_id=? AND h.approval_status='APPROVED' AND d.status='ACTIVE'),0)+COALESCE((SELECT SUM(confirmed_cost_amount) FROM partner_settlement WHERE project_id=? AND status='APPROVED'),0)+COALESCE((SELECT SUM(loss_confirmed_amount) FROM fin_deposit WHERE project_id=?),0)) amount`,[plan.projectId,plan.projectId,plan.projectId]);basisAmount=Math.max(0,Number(incomeRows[0]?.amount??0)-Number(costRows[0]?.amount??0))
          }
          const[historyRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(net_settlement_amount),0) amount FROM partner_settlement WHERE plan_id=? AND status IN('APPROVED','PAID')`,[plan.planId]);const historical=Number(historyRows[0]?.amount??0)
          const calculated=calculateSettlement({basis:plan.basis,basisCents:toCents(basisAmount),ratioPpm:plan.ratio==null?null:BigInt(Math.round(Number(plan.ratio)*1_000_000)),fixedCents:plan.fixedAmount==null?null:toCents(plan.fixedAmount),historicalSettledCents:toCents(historical),deductionCents:toCents(input.deductionAmount),lowerLimitCents:plan.lowerLimit==null?null:toCents(plan.lowerLimit),upperLimitCents:plan.upperLimit==null?null:toCents(plan.upperLimit)})
          const snapshot={planVersionId:String(plan.versionId),versionNumber:plan.versionNumber,basis:plan.basis,basisAmount,ratio:plan.ratio,fixedAmount:plan.fixedAmount,deductibleCostScope:plan.deductibleScope,roundingRule:plan.roundingRule,calculated:{theoretical:fromCents(calculated.theoreticalCents),available:fromCents(calculated.availableCents),net:fromCents(calculated.netSettlementCents)}};const snapshotJson=JSON.stringify(snapshot);const hash=createHash('sha256').update(snapshotJson).digest('hex');const code=await allocateNumber(connection,'PARTNER_SETTLEMENT')
          const[result]=await connection.execute<ResultSetHeader>(`INSERT INTO partner_settlement(settlement_code,project_id,plan_id,plan_version_id,partner_id,period_start_on,period_end_on,basis_amount_snapshot,ratio_snapshot,rule_snapshot,gross_settlement_amount,historical_settled_amount,deduction_amount,net_settlement_amount,rounding_difference,invoice_requirement,snapshot_sha256,created_by,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[code,plan.projectId,plan.planId,plan.versionId,plan.partnerId,input.periodStartOn,input.periodEndOn,basisAmount,plan.ratio,snapshotJson,fromCents(calculated.theoreticalCents),historical,input.deductionAmount,fromCents(calculated.netSettlementCents),'0.00',input.invoiceRequirement??null,hash,user.id,user.id]);return{id:String(result.insertId),code,snapshot}
        }
        case 'deposit.create': {
          const code=await allocateNumber(connection,'DEPOSIT');const[result]=await connection.execute<ResultSetHeader>(`INSERT INTO fin_deposit(deposit_code,project_id,bid_id,contract_id,deposit_type,direction,counterparty_id,amount,due_payment_on,due_return_on,account,created_by,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`,[code,input.projectId,input.bidId??null,input.contractId??null,input.depositType,input.direction,input.counterpartyId,input.amount,input.duePaymentOn,input.dueReturnOn??null,input.account??null,user.id,user.id]);return{id:String(result.insertId),code}
        }
        case 'deposit.event.create': {
          const[old]=await connection.execute<RowDataPacket[]>(`SELECT amount,occupied_amount occupied,loss_confirmed_amount loss,status FROM fin_deposit WHERE id=? FOR UPDATE`,[input.depositId]);const deposit=old[0];if(!deposit)throw new AppError('DEPOSIT_NOT_FOUND','保证金不存在',404)
          const[seen]=await connection.execute<RowDataPacket[]>(`SELECT id FROM fin_deposit_event WHERE idempotency_key=?`,[input.idempotencyKey]);if(seen[0])return{idempotent:true}
          let occupied=Number(deposit.occupied),loss=Number(deposit.loss),status=deposit.status as string
          if(input.eventType==='PAY'){const[paidRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(amount),0) amount FROM fin_deposit_event WHERE deposit_id=? AND event_type='PAY'`,[input.depositId]);if(Number(paidRows[0]?.amount??0)+Number(input.amount)>Number(deposit.amount))throw new AppError('DEPOSIT_AMOUNT_EXCEEDED','累计缴纳金额超过保证金登记金额',409);occupied+=Number(input.amount);status='PAID'}else if(input.eventType==='RETURN'){if(Number(input.amount)>occupied)throw new AppError('DEPOSIT_BALANCE_EXCEEDED','退回金额超过资金占用',409);occupied-=Number(input.amount);status=occupied===0?'RETURNED':'PENDING_RETURN'}else if(input.eventType==='FORFEIT'){if(Number(input.amount)>occupied)throw new AppError('DEPOSIT_BALANCE_EXCEEDED','没收金额超过资金占用',409);occupied-=Number(input.amount);status='FORFEITED'}else if(input.eventType==='CONFIRM_LOSS'){if(!input.lossApprovalPassed)throw new AppError('DEPOSIT_LOSS_APPROVAL_REQUIRED','损失确认必须审批通过',409);const[forfeitRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(amount),0) amount FROM fin_deposit_event WHERE deposit_id=? AND event_type='FORFEIT'`,[input.depositId]);if(loss+Number(input.amount)>Number(forfeitRows[0]?.amount??0))throw new AppError('DEPOSIT_LOSS_EXCEEDED','损失确认金额超过已没收金额',409);loss+=Number(input.amount);status='FORFEITED'}else{if(occupied!==0)throw new AppError('DEPOSIT_OCCUPIED','仍有资金占用不能作废',409);status='VOID'}
          await connection.execute(`INSERT INTO fin_deposit_event(deposit_id,event_type,amount,occurred_on,description,operator_id,idempotency_key) VALUES(?,?,?,?,?,?,?)`,[input.depositId,input.eventType,input.amount,input.occurredOn,input.description??null,user.id,input.idempotencyKey]);await connection.execute(`UPDATE fin_deposit SET occupied_amount=?,loss_confirmed_amount=?,status=?,paid_on=CASE WHEN ?='PAY' THEN ? ELSE paid_on END,returned_on=CASE WHEN ?='RETURN' AND ?=0 THEN ? ELSE returned_on END,version=version+1,updated_by=? WHERE id=?`,[occupied,loss,status,input.eventType,input.occurredOn,input.eventType,occupied,input.occurredOn,user.id,input.depositId]);return{idempotent:false,status,occupiedAmount:occupied,lossConfirmedAmount:loss}
        }
        case 'project.close.create': {
          const[acceptanceRows]=await connection.execute<RowDataPacket[]>(`SELECT COUNT(*) count FROM prj_acceptance WHERE project_id=? AND status='COMPLETED'`,[input.projectId]);const acceptancePassed=Number(acceptanceRows[0]?.count??0)>0
          const[contractRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(tax_exclusive_amount),0) amount FROM con_contract WHERE project_id=? AND contract_type='INCOME' AND amount_status='CONFIRMED' AND status NOT IN('VOID','REJECTED')`,[input.projectId]);const contractAmount=Number(contractRows[0]?.amount??0)
          const[invoiceRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(tax_inclusive_amount),0) amount FROM fin_sales_invoice WHERE project_id=? AND is_reversed=0`,[input.projectId]);const invoicedAmount=Number(invoiceRows[0]?.amount??0)
          const[receiptRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(amount),0) amount FROM fin_receipt WHERE project_id=? AND status='ACTIVE'`,[input.projectId]);const receivedAmount=Number(receiptRows[0]?.amount??0)
          const[depositRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(occupied_amount),0) amount FROM fin_deposit WHERE project_id=?`,[input.projectId]);const unreturnedDeposit=Number(depositRows[0]?.amount??0)>0
          const[riskRows]=await connection.execute<RowDataPacket[]>(`SELECT COUNT(*) count FROM prj_risk_issue WHERE project_id=? AND is_deleted=0 AND status NOT IN('CLOSED')`,[input.projectId]);const openIssues=Number(riskRows[0]?.count??0)>0
          const[settlementRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(net_settlement_amount),0) amount FROM partner_settlement WHERE project_id=? AND status IN('APPROVED','PAID')`,[input.projectId]);const settlementAmount=Number(settlementRows[0]?.amount??0)
          const[costRows]=await connection.execute<RowDataPacket[]>(`SELECT (COALESCE((SELECT SUM(d.amount) FROM fin_reimbursement_detail d JOIN fin_reimbursement h ON h.id=d.reimbursement_id WHERE h.project_id=? AND h.approval_status='APPROVED' AND d.status='ACTIVE'),0)+COALESCE((SELECT SUM(confirmed_cost_amount) FROM partner_settlement WHERE project_id=? AND status IN('APPROVED','PAID')),0)+COALESCE((SELECT SUM(loss_confirmed_amount) FROM fin_deposit WHERE project_id=?),0)) amount`,[input.projectId,input.projectId,input.projectId]);const confirmedCost=Number(costRows[0]?.amount??0)
          const check={acceptancePassed,archivePassed:Boolean(input.archiveCheckPassed),outstandingReceivable:receivedAmount<contractAmount,unreturnedDeposit,openIssues};validateProjectClose(check,input.closeType,input.openItems,user.roleCodes.includes('COMPANY_PRINCIPAL')?'COMPANY_PRINCIPAL':undefined)
          const code=await allocateNumber(connection,'PROJECT_CLOSE');const profit={contractOperatingProfit:contractAmount-confirmedCost,cashContribution:receivedAmount};const[result]=await connection.execute<ResultSetHeader>(`INSERT INTO prj_close_application(close_code,project_id,applied_on,completion_summary,acceptance_conclusion,contract_amount_snapshot,invoiced_amount_snapshot,received_amount_snapshot,confirmed_cost_snapshot,settlement_amount_snapshot,archive_check_passed,profit_snapshot,close_description,close_type,special_approval_comment,created_by,updated_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[code,input.projectId,input.appliedOn,input.completionSummary,input.acceptanceConclusion,contractAmount,invoicedAmount,receivedAmount,confirmedCost,settlementAmount,input.archiveCheckPassed,JSON.stringify(profit),input.closeDescription,input.closeType,input.specialApprovalComment??null,user.id,user.id]);for(const item of input.openItems as Array<Record<string,any>>){await connection.execute(`INSERT INTO prj_close_open_item(close_application_id,item_type,description,responsible_id,due_on) VALUES(?,?,?,?,?)`,[result.insertId,item.type,item.description,item.responsibleId,item.dueOn])}return{id:String(result.insertId),code,check,profit}
        }
        case 'settlement.summary': {
          const all=user.dataScopes.some((scope)=>scope.type==='ALL'),access=`(?=1 OR EXISTS(SELECT 1 FROM prj_project p LEFT JOIN prj_project_member m ON m.project_id=p.id AND m.status='ACTIVE' WHERE p.id=x.project_id AND (p.project_manager_id=? OR m.employee_id=?)))`
          const[planRows]=await connection.execute<RowDataPacket[]>(`SELECT COUNT(*) count FROM partner_plan x WHERE x.is_deleted=0 AND ${access}`,[all?1:0,user.employeeId,user.employeeId])
          const[settlementRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(x.net_settlement_amount),0) amount FROM partner_settlement x WHERE x.status IN('APPROVED','PAID') AND ${access}`,[all?1:0,user.employeeId,user.employeeId])
          const[depositRows]=await connection.execute<RowDataPacket[]>(`SELECT COALESCE(SUM(x.occupied_amount),0) amount FROM fin_deposit x WHERE ${access}`,[all?1:0,user.employeeId,user.employeeId])
