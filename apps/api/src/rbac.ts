...
export function requireEnabledUser(user: SessionUser | null): asserts user is SessionUser {
  if (!user) throw new UnauthorizedError()
  if (!user.enabled) throw new ForbiddenError('内部账号已停用')
}

export function requirePermission(user: SessionUser, permissionCode: string): void {
...
export function requireDataAccess(user: SessionUser, resource: ResourceContext): void {
